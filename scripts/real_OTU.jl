...
int_fit_orig = hdmm(
X, otus, y, grp;
penalty="scad", λ=100, ψstr="ident", control=control
)
# check diagnostics
scatter(int_fit_orig.fitted, int_fit_orig.resid)
scatter(otus[:,365], y)
mask_outly = int_fit_orig.resid .> 3
scatter(int_fit_orig.fitted, y)
annotate!(
...
Random.seed!(1234) 
@time int_fit = hdmm(
X[.!mask_outly,:], otus[.!mask_outly,:], y[.!mask_outly], grp[.!mask_outly]; 
penalty="scad", λ=100, ψstr="ident", control=control
)
scatter(int_fit.fitted, int_fit.resid)
scatter(int_fit.fitted, y[.!mask_outly])
