...
new_gwas_results <- read.csv("../sim_results/gwas_new/best_results.csv")
...
Now we look at true positive rates. We can plot the percentage of datasets in which we achieved perfect recovery. First prepare the data.

```{r}
gene_main_tp_data <- bgr_nz10 %>%
  filter(q==3) %>%
  select(penalty, cov_str, predictor_cor, tp) %>% 
  group_by(penalty, predictor_cor, cov_str) %>%
  summarize(
    tp_3_to_6 = sum(tp >= 3 & tp < 6),
    tp_6_to_9 = sum(tp >= 6 & tp <= 9),
    tp_10 = sum(tp == 10)
  ) %>%
  ungroup() %>%
  mutate(
    tp_total = tp_3_to_6 + tp_6_to_9 + tp_10,  # Calculate total tp count
    tp_3_to_6_percent = round(tp_3_to_6 / tp_total,2),  # Calculate percentage for tp_3_to_6
    tp_6_to_9_percent =  round(tp_6_to_9 / tp_total,2),  # Calculate percentage for tp_6_to_9
    tp_10_percent = round(tp_10 / tp_total,2)           # Calculate percentage for tp_10
  ) 

print(gene_main_tp_data)
```

Now plot
```{r}
x_order <- c("scalar.No", "diagonal.No", "unstructured.No", "scalar.Yes", "diagonal.Yes", "unstructured.Yes")
gene_main_tp <- gene_main_tp_data %>%
  mutate(cor_label = ifelse(predictor_cor == "No", 
                            "no correlation", "yes correlation")) %>%
  unite("x_combo", cov_str, predictor_cor, sep = ".") %>%
  mutate(x_combo = factor(x_combo, x_order)) %>%
  ggplot(aes(x_combo, tp_10_percent, fill=penalty)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~cor_label, scales = "free_x") + # Separate by correlation label
  labs(x = "", y = "True Positive = 10 (%)", fill = "penalty", title = "") +
  scale_fill_manual(values = c("LASSO" = "orange", "SCAD" = "royalblue")) +
  theme_minimal() +
...
```{r, warning=FALSE}
gene_main_fp + gene_main_tp + plot_annotation(tag_levels = 'A') & theme(legend.position = 'bottom') 
ggsave("../plots/gene/gene_main_select.eps", width = 40, height = 15, units = "cm")
```
...
gene_supp_nz5_fp
ggsave(
    "../plots/gene/gene_supp_nz5_fp.eps",
    gene_supp_nz5_fp,
    width = 15,
    height = 15,
    units = "cm"
...
gene_supp_nz5_tp
ggsave(
    "../plots/gene/gene_supp_nz5_tp.eps",
    gene_supp_nz5_tp,
    width = 15,
    height = 15,
    units = "cm"
...
#Note that there are fewer than 100 data-sets for some of the lasso settings (but none of the scad settings)!
ggsave(
  "../plots/gene/gene_supp_nz10_fp.eps",
  gene_supp_nz10_fp,
  width = 15,
  height = 15,
  units = "cm"
...
#Note that there are fewer than 100 data-sets for some of the lasso settings (but none of the scad settings)!
ggsave(
  "../plots/gene/gene_supp_nz10_tp.eps",
  gene_supp_nz10_tp,
  width = 15,
  height = 15,
  units = "cm"
...
ggsave("../plots/gene/gene_supp_select.eps", width = 40, height = 30, units = "cm")
```
...
#Note that there are fewer than 100 data-sets for some of the lasso settings (but none of the scad settings)!
ggsave(
  "../plots/gene/gene_main_beta2.eps",
  gene_main_beta2,
  width = 20,
  height = 15,
  units = "cm"
...
#Note that there are fewer than 100 data-sets for some of the lasso settings (but none of the scad settings)!
ggsave(
  "../plots/gene/gene_main_beta3.eps",
  gene_main_beta3,
  width = 20,
  height = 15,
  units = "cm"
...
gene_main_beta4
ggsave(
  "../plots/gene/gene_main_beta4.eps",
  gene_main_beta4,
  width = 20,
  height = 15,
  units = "cm"
...
ggsave("../plots/gene/gene_main_est.eps", width = 40, height = 15, units = "cm")
```
...
Let's get a summary of coefficient estimates under SCAD penalty for later use (see section "Inter-omics comparison":

```{r}

coef_vals <- c(-1,5,-3,2,2)

extract_coef <- function(coef_name, df) {
  coef_index = as.integer(str_extract(coef_name, "\\d+"))
  coef_val = coef_vals[coef_index - 5]
  coef = as.name(coef_name)
  coef_df <- df %>%
    filter(penalty == "SCAD") %>%
    select(setting, data_id, {{coef}}) %>%
    mutate(identified = {{coef}}!=0) %>%
    filter(identified) %>%
    group_by(setting) %>%
    summarise(recovered = n(), mean = mean({{coef}}), 
              bias = mean - coef_val, sd = sd({{coef}})) %>%
    mutate(coef = coef_name)
  return(coef_df)
}

gene_coefs <- map(paste("beta", 6:10, sep = "_"),
                  extract_coef, df = bgr_nz10) %>%
  list_rbind()
...
#Note that there are fewer than 100 data-sets for some of the lasso settings (but none of the scad settings)!
ggsave(
  "../plots/gene/gene_supp_beta3.eps",
  gene_supp_beta3,
  width = 15,
  height = 15,
  units = "cm"
...
gene_supp_nz5_beta3
ggsave(
    "../plots/gene/gene_supp_nz5_beta3.eps",
    gene_supp_nz5_beta3,
    width = 15,
    height = 15,
    units = "cm"
...
gene_supp_nz5_beta4
ggsave(
    "../plots/gene/gene_supp_nz5_beta4.eps",
    gene_supp_nz5_beta4,
    width = 15,
    height = 15,
    units = "cm"
...
ggsave("../plots/gene/gene_supp_est.eps", width = 40, height = 40, units = "cm")
```
...
ggsave("../plots/gene/var_ge_comp.eps", width = 40, height = 30, units = "cm")
```
...
gwas_main_tp_data <- best_gwas_results %>%
  ungroup() %>%
  filter(q==3) %>%
...
print(gwas_main_tp_data)
```

Now plot
```{r}
gwas_main_tp <-  ggplot(gwas_main_tp_data, 
                        aes(cov_str, tp_10_percent, fill=penalty)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "", y = "True Positive = 10 (%)", fill = "penalty", title = "") +
  scale_fill_manual(values = c("LASSO" = "orange", "SCAD" = "royalblue")) +
  theme_minimal() +
...
```{r}
gwas_main_fp + gwas_main_tp + plot_annotation(tag_levels = 'A') & theme(legend.position = 'bottom') 
ggsave("../plots/gwas/gwas_main_select.eps", width = 40, height = 20, units = "cm")
```
...
ggsave("../plots/gwas/gwas_supp_select.eps", width = 40, height = 10, units = "cm")
```
...
#Note that there are fewer than 100 data-sets for some of the lasso settings (but none of the scad settings)!
ggsave(
  "../plots/gwas/gwas_main_beta3.eps",
  gwas_main_beta3,
  width = 20,
  height = 8,
  units = "cm"
...
#Note that there are fewer than 100 data-sets for some of the lasso settings (but none of the scad settings)!
ggsave(
  "../plots/gwas/gwas_main_beta4.eps",
  gwas_main_beta4,
  width = 20,
  height = 8,
  units = "cm"
...
ggsave("../plots/gwas/gwas_main_est.eps", width = 40, height = 10, units = "cm")
...
#Note that there are fewer than 100 data-sets for some of the lasso settings (but none of the scad settings)!
ggsave(
  "../plots/gwas/gwas_supp_beta3.eps",
  gwas_supp_beta3,
  width = 20,
  height = 8,
  units = "cm"
...
```{r}
df_true <- data.frame(y_value = 3)
gwas_supp_beta4 <- best_gwas_results %>%
  filter(q != 3) %>%
  group_by(penalty, q) %>%
  mutate(outlier = beta_4 > quantile(beta_4, .75) + IQR(beta_4) * 1.5 | 
           beta_4 < quantile(beta_4, .25) - IQR(beta_4) * 1.5) %>%
...
#Note that there are fewer than 100 data-sets for some of the lasso settings (but none of the scad settings)!
ggsave(
  "../plots/gwas/gwas_supp_beta4.eps",
  gwas_supp_beta4,
  width = 20,
  height = 8,
  units = "cm"
...
```{r}
gwas_supp_beta3 + gwas_supp_beta4 + plot_layout(guides = "collect") + plot_annotation(tag_levels = 'A') & theme(legend.position = 'bottom') 
ggsave("../plots/gwas/gwas_supp_est.eps", width = 40, height = 10, units = "cm")

```

Let's get coefficients estimates in all GWAS settings under SCAD penalty for later use (see section "Inter-omics comparison":
```{r}
gwas_coefs <- map(paste("beta", 6:10, sep = "_"),
                  extract_coef, df = best_gwas_results) %>%
  list_rbind()
...
For variance components, we'll use the results from the simulations in which we didn't standardize the random effects covariance matrix, as this messed up the estimates of variance components in the original simulations.

```{r}
best_new_gwas_results <- new_gwas_results %>%
  group_by(setting, data_id) %>%
  arrange(bic, .by_group = TRUE) %>%
  filter(row_number() == 1) %>%
  mutate(
...
var_gwas_scalar <- best_new_gwas_results %>%
  filter(cov_str == "scalar") %>%
  ggplot(aes(y = psi_1, x=q)) +
  geom_boxplot(position = position_dodge(width = 0.9), aes(col = penalty), outlier.shape = 21)  +
  geom_hline(show.legend = FALSE,data = df_true, aes(yintercept = y_value, linetype = "True Parameter")) +
...
ggsave(
  "../plots/gwas/var_gwas_scalar.eps",
  var_gwas_scalar,
  width = 20,
  height = 15,
  units = "cm"
...
var_gwas_diag <- best_new_gwas_results %>%
  filter(cov_str == "diagonal") %>%
  pivot_longer(cols = c(psi_1, psi_5, psi_9), 
               names_to = "parameter", values_to = "estimate") %>%
  ggplot(aes(parameter, estimate)) +
...
Lsym <- matrix(c(sqrt(3), 0 ,0, 1, sqrt(3), 0, -sqrt(2), 1, sqrt(2)), 
               nrow = 3, byrow = T)
df_true <- data.frame(parameter = rep(1:9, 2), 
                      y_value = rep(as.vector(Lsym %*% t(Lsym)), 2))

var_gwas_sym <- best_new_gwas_results %>%
  filter(cov_str == "unstructured") %>%
  pivot_longer(cols = paste0("psi_", 1:9), 
               names_to = "parameter", values_to = "estimate") %>%
  ggplot(aes(parameter, estimate)) +
...
ggsave("../plots/gwas/var_gwas_comp.eps", width = 40, height = 30, units = "cm")
```
...
otu_main_fp <- best_otu_results  %>%
  ...
  facet_grid2(otu_cor ~ cov_str, scales = "free_y",
             labeller = labeller(predictor_cor = corr.labs)) +
  labs(y = "false positive rate", x = TeX("$\\Psi_\\theta$ structure"), col = "network") + 
  theme(legend.position = "top",
        axis.text.x=element_blank(),
        axis.ticks.x = element_blank(),
        axis.title.x = element_blank(),
...
otu_main_tp_data <- best_otu_results %>%
  ungroup() %>%
  filter(q==3) %>%
  select(otu_cor, cov_str, tp) %>% 
  group_by(otu_cor, cov_str) %>%
  summarize(
    tp_3_to_6 = sum(tp >= 3 & tp < 6),
    tp_6_to_10 = sum(tp >= 6 & tp <= 10),
    tp_11 = sum(tp == 11)
  ) %>%
  ungroup() %>%
  mutate(
    tp_total = tp_3_to_6 + tp_6_to_10 + tp_11,  # Calculate total tp count
    tp_3_to_6_percent = round(tp_3_to_6 / tp_total,2),  # Calculate percentage for tp_3_to_6
    tp_6_to_10_percent =  round(tp_6_to_10 / tp_total,2),  # Calculate percentage for tp_6_to_9
    tp_11_percent = round(tp_11 / tp_total,2)           # Calculate percentage for tp_10
  ) 

print(otu_main_tp_data)
```

Now plot
```{r}
otu_main_tp <-  otu_main_tp_data %>%
  filter(otu_cor %in% c("band", "scale_free")) %>%
  ggplot(aes(cov_str, tp_11_percent, fill=otu_cor)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "", y = "True Positive = 11 (%)", fill = "network", title = "") +
  scale_fill_manual(values = c("band" = "seagreen3", "scale_free" = "purple1")) +
  theme_minimal() +
  theme(legend.position = "top", text = element_text(size = 18))
...
```{r}
otu_main_fp + otu_main_tp + plot_annotation(tag_levels = 'A') & theme(legend.position = 'bottom') 
ggsave("../plots/otu/otus_main_select.eps", width = 40, height = 20, units = "cm")
```
...
otu_supp_fp_others
ggsave(
  "../plots/otu/otu_supp_fp_others.eps",
  otu_supp_fp_bsf,
  width = 20,
  height = 10,
  units = "cm"
...
otu_supp_tp_others
ggsave(
  "../plots/otu/otu_supp_tp_others.eps",
  otu_supp_fp_bsf,
  width = 20,
  height = 10,
  units = "cm"
...
ggsave("../plots/otu/otu_supp_select_bsf.eps", width = 40, height = 15, units = "cm")
```


```{r}
otu_supp_fp_others + otu_supp_tp_others + plot_layout(guides = "collect") + plot_annotation(tag_levels = 'A')
ggsave("../plots/otu/otu_supp_select_others.eps", width = 40, height = 15, units = "cm")
```
...
ggsave(
  "../plots/otu/otu_main_betagrp.eps",
  otu_main_betagrp,
  width = 20,
  height = 10,
  units = "cm"
...
ggsave(
  "../plots/otu/otu_supp_betagrp_bsf.eps",
  otu_supp_betagrp_bsf,
  width = 20,
  height = 10,
  units = "cm"
...
ggsave(
  "../plots/otu/otu_supp_betagrp_other.eps",
  otu_supp_betagrp_other,
  width = 20,
  height = 10,
  units = "cm"
...
Let's get coefficient estimates in all OTU settings for future usee (see section "Inter-omics comparison":
```{r}

extract_coef_otu <- function(coef_name){
  coef = as.name(coef_name)
  # OTU starts with beta_7, so must subtract 1
  coef_index = as.integer(str_extract(coef_name, "\\d+")) - 1
  coef_new_name = paste0("beta_", coef_index)
  coef_val = coef_vals[coef_index - 5] 
  coef_df <- best_otu_results %>%
    select(setting, otu_cor, data_id, {{coef}}) %>%
    mutate(identified = {{coef}}!=0) %>%
    filter(identified) %>%
    group_by(setting, otu_cor) %>%
    summarise(recovered = n(), mean = mean({{coef}}), 
              bias = mean - coef_val, sd = sd({{coef}})) %>%
    mutate(coef = coef_new_name)
  return(coef_df)
}

otu_coefs <- map(paste("beta", 7:11, sep = "_"),
                  extract_coef_otu) %>%
  list_rbind()
...
coef_results <- list(gene = gene_coefs, 
     gwas = gwas_coefs, 
     otu = otu_coefs) %>%
  bind_rows(.id = "dtype") %>%
  #filter(coef == "beta_8") %>%
  mutate(cov_str = recode_factor(
    ...
    coef = factor(coef, 
                  levels = paste0("beta_", 6:10),
                  labels = paste0("beta[", 6:10, "]")
                  )
    ) %>%
...
All true positives
```{r}
ggplot(coef_results, aes(cov_str, 100-recovered, color = dtype)) +
...
```{r}
all_sds <- coef_results %>%
  filter(grouping %in% c("gwas", "gene:correlated", "otu:band")) %>%
  ggplot(aes(cov_str, sd, color = grouping, group = grouping)) +
  geom_line() +
  geom_point() +
  facet_wrap( ~ coef, labeller=label_parsed) +
  scale_x_discrete(labels = c("q=1", 
                              "q=3:\nscalar", 
                              "q=3:\ndiag",
                              "q=3:\nunstruct",
                              "q=5")) +
  ...
  labs(
    color = "Omics type",
    x = "Random effect covariance structure", 
    y = TeX("$SD(\\hat{\\beta} | \\hat{\\beta} \\neq 0)$")) +
  theme(legend.position = "top")
