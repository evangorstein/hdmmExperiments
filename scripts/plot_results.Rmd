...
var_gwas_diag <- best_gwas_results %>%
  filter(cov_str == "diagonal") %>%
  left_join(variances_diag, by = "data_id") %>%
...
```{r, cache=TRUE, message=FALSE}
unzip("../data/GWAS/random3_covsym.zip", exdir = "../data/GWAS")
idxs = 1:100
sym_dfs <- idxs %>% map( 
